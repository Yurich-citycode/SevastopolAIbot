...
- 🚀 [START_HERE.md](START_HERE.md) — пошаговый запуск для не-программиста: токен, Colab, VPS, Cloudflare Worker, чек-лист проверки
- 📦 [FILL_PACK.md](FILL_PACK.md) — готовые блоки данных для вставки в живую Google-таблицу по колонкам
...
| `Sevastopol AI Архив событий.xlsx` | Прошедшие события — на память, бот их не читает |
| `requirements.txt` | Зависимости бота |
| `.env.example` | Шаблон настроек (скопировать в `.env`, `chmod 600`) |
| `Dockerfile`, `.dockerignore` | Запуск в контейнере |
| `START_HERE.md` | Инструкция «с нуля» для владельца бота |
| `FILL_PACK.md` | Что и как вставить в Google-таблицу руками |
...
| `tools/setup_env.py` | Мастер первого запуска: создаёт `.env`, проверяет токен |
...
`tools/rebuild_zip.py`) и `AUDIT_REPORT.md` тоже удалены — данные правятся прямо
в Google-таблице. Лист «События» наполняется вручную, прошедшие события лежат
в `Sevastopol AI Архив событий.xlsx`. Кнопка «📰 Новости города» осталась: она
ведёт на канал `NEWS_CHANNEL_URL`.
...
- **Дата** события — `ДД.ММ.ГГГГ`. События в прошлом бот не показывает, поэтому
  лист «События» нужно регулярно обновлять вручную; готовые блоки для вставки
  лежат в [FILL_PACK.md](FILL_PACK.md), старые события — в
  `Sevastopol AI Архив событий.xlsx`.
...
- Колонки бот ищет по шапке, поэтому порядок колонок можно менять. Шапки
  с латинской `B` («Bремя работы», «Bконтакте») бот ещё понимает, но в таблице
  они уже исправлены на русскую `В` — лучше держать именно так.
...
Пополнение таблицы — руками через Google-таблицу (бот подхватит изменения сам).
Готовые блоки значений по колонкам и пошаговая инструкция — в
[FILL_PACK.md](FILL_PACK.md). Файл `Sevastopol AI База.xlsx` в репозитории —
просто актуальная копия базы для истории; импортировать его поверх
Google-таблицы нельзя, там формулы `XLOOKUP` в колонке «Меню Справочник».
...
aiogram — заглушки. Счётчиков «сколько чего должно быть в базе» в тестах нет:
проверяется качество — что в каждой категории есть данные, что все карточки
собираются и что лимиты Telegram не превышены. Поэтому тесты проходят при
любом наполнении базы; ожидаемый результат — ни одного `❌` и строка
`ИТОГ: N прошло, 0 упало`.
